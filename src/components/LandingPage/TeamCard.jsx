...
const TeamCard = ({ image, name, description, github, linkedin }) => (
  <div className="relative group w-80 md:w-100 rounded-3xl overflow-hidden shadow-lg transition-all duration-500 transform hover:scale-100 hover:shadow-[0px_7px_12px_rgba(59,130,246,0.9)] bg-opacity-30 backdrop-blur-lg border border-gray-700">
    <img
      src={image}
      alt={name}
    ...
    <div className="absolute bottom-0 w-full px-6 py-5 bg-black/40 backdrop-blur-xl rounded-b-3xl transition-all duration-300 group-hover:bg-black/50">
      <h2 className="text-2xl font-bold text-white tracking-wide drop-shadow-lg text-center">{name}</h2>
...
const TechSavvyTeam = () => (
  <div className="flex flex-col items-center justify-center gap-10 py-5">
    <h1 className="text-4xl md:text-6xl font-extrabold text-center tracking-tight text-white">
      Meet Our
      <span className="text-transparent bg-clip-text bg-blue-500 drop-shadow-md">
        {" "}Tech-Savvy Team
      </span>
    </h1>
